...
class MultiPolygonTest {
    ...
    fun bbox_doesNotSerializeWhenNotPresent() {
        ...
        val actualMultiPolygon = MultiPolygon.fromJson(multiPolygon.json())
    ...
    fun passingInSinglePolygon_doesHandleCorrectly() {
        ...
        assertEquals(
            2.0,
            multiPolygon.coordinates.first().first().first().latitude,
            DELTA
        )
    }
    ...
    fun fromJson() {
        ...
        assertEquals(
            geo.coordinates.first().first().first().longitude,
            102.0,
            DELTA
        )
        assertEquals(
            geo.coordinates.first().first().first()
                .latitude, 2.0, DELTA
        )
